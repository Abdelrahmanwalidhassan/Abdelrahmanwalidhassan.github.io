window.addEventListener('DOMContentLoaded', async () => {
			...
			reader.onload = (e) => {
				...
				try {
					if (event.target.id === 'guild-data-input') {
						...
						if (!(guildData.id || guildData.channels || guildData.roles)) return alert('Invalid data format');
						document.getElementById('con').style.display = 'block';
					} else if (event.target.id === 'panel-data-input') idData = JSON.parse(atob(base64String));
	...
	async function update() {
		console.log(idData, guildData);
		name.value = idData.name || '';
		description.value = idData.description || '';
		status.value = (idData.status || 'open').charAt(0).toUpperCase() + (idData.status || 'open').slice(1);
		schedule_status.value = idData.schedule?.enabled ? 'Enabled' : 'Disabled';
		schedule_open.value = addLeadingZeros(idData.schedule?.open || 1, 2) + ':00';
		schedule_close.value = addLeadingZeros(idData.schedule?.close || 23, 2) + ':00';
		schedule_utc_offset.value =
			(idData.schedule || 0)?.utcOffset > 0 ? '+' + idData.schedule?.utcOffset : idData.schedule?.utcOffset || 0;
		...
		creation_form_status.value = idData.creationForm?.enabled ? 'true' : 'false';
		...
		role_select.addEventListener('change', (e) => {
			role_option.innerHTML = guildData.roles
				.map(
					(r) =>
						`<div class="gender">` +
						`<input type="checkbox" name="role-option-select" id="${r.id}" ${idData.roles?.[e.target.value]?.includes(r.id) ? 'checked' : ''} />` +
			...
			document.getElementsByName('role-option-select').forEach((e) =>
				e.addEventListener('change', (e) => {
					...
					else idData.roles[role] = idData.roles[role].filter((r) => r !== e.target.id);
					console.log(idData);
				})
			);
		});
		...
		[state_select, state_role_select].forEach((e) => e.addEventListener('change', updatePermSelect));
		...
		function updatePermSelect() {
			...
			perms.innerHTML = TextChannelPermissions.map(
				(r) =>
					`<div class="gender">` +
					`<input type="checkbox" name="perm" id="${state + '-' + role + '-' + r}" ${idData.permissions?.[state]?.[role]?.includes(r) ? 'checked' : ''}/>` +
			...
			document.getElementsByName('perm').forEach((e) =>
				e.addEventListener('change', (ee) => {
					...
					console.log(!idData.permissions?.[state]?.[role]?.includes(ee.target.id.split('-')[2]));

					if (ee.target.checked && !idData.permissions?.[state]?.[role]?.includes(ee.target.id.split('-')[2])) {
						if (!idData.permissions) idData.permissions = {};
						if (!idData.permissions[state]) idData.permissions[state] = {};
						if (!idData.permissions[state][role]) idData.permissions[state][role] = [];

						idData.permissions[state][role].push(ee.target.id.split('-')[2]);
					} else if (
						!ee.target.checked &&
						idData.permissions?.[state]?.[role]?.includes(ee.target.id.split('-')[2])
					) {
						idData.permissions[state][role] = idData.permissions[state][role].filter(
							(d) => d !== ee.target.id.split('-')[2]
						);
					}
		...
		function createQuestionElement(index) {
            ...
            <div class="input-box">
                ...
                <input id="question-name-${index}" type="text" placeholder="Enter Question Name" value="${question.name}" required />
            </div>
            <div class="input-box">
                <label for="question-placeholder-${index}">Placeholder</label>
                <input id="question-placeholder-${index}" type="text" placeholder="Enter Placeholder" value="${question.placeholder}" required />
            </div>
            <div class="input-box">
                <label for="question-required-${index}">Required</label>
                <input id="question-required-${index}" type="checkbox" ${question.required ? 'checked' : ''} />
            </div>
            <div class="input-box">
                <label for="question-max-length-${index}">Max Length</label>
                <input id="question-max-length-${index}" type="number" value="${question.max_length}" required />
            </div>
            <div class="input-box">
                <label for="question-min-length-${index}">Min Length</label>
                <input id="question-min-length-${index}" type="number" value="${question.min_length}" required />
            </div>
            <div class="input-box">
                <label for="question-multi-line-${index}">Multi Line</label>
                <input id="question-multi-line-${index}" type="checkbox" ${question.multi_line ? 'checked' : ''} />
            </div>
			...
			questionDiv.querySelectorAll('input, select').forEach((input) => {
				input.addEventListener('input', () => {
					...
					questions[index] = {
						...
						placeholder: document.getElementById(`question-placeholder-${index}`).value,
						required: document.getElementById(`question-required-${index}`).checked,
						max_length: parseInt(document.getElementById(`question-max-length-${index}`).value, 10),
						min_length: parseInt(document.getElementById(`question-min-length-${index}`).value, 10),
						multi_line: document.getElementById(`question-multi-line-${index}`).checked
					};
					logFormOptions();
				});
			});
	...
	days?.forEach((e) => {
		...
		e.addEventListener('click', (ee) => {
			if (ee.target.checked && (!idData.schedule || idData.schedule.days?.includes(ee.target.id))) {
				idData.schedule = {
					days: [ee.target.id]
				};
			} else if (ee.target.checked && !idData.schedule?.days?.includes(ee.target.id)) {
				idData.schedule.days.push(ee.target.id);
			} else if (!ee.target.checked && idData.schedule.days.includes(ee.target.id)) {
				idData.schedule.days = idData.schedule.days.filter((d) => d !== ee.target.id);
			}
		});
	});
